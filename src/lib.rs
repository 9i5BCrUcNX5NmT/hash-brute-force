pub mod main_logic {
    ...
    pub fn compute_result(null: usize, find: usize) -> Vec<(usize, String)> {
        ...
        while need_to_contunue(find, Arc::clone(&counter)) {
        ...
        rx.iter().take(find).collect::<Vec<(usize, String)>>()
    }
    ...
    fn hash_of_num_in_thread(
        ...
        // start new thread
        pool.execute(move || {
            if need_to_contunue(find, Arc::clone(&counter)) {
    ...
    fn need_to_contunue(find: usize, counter: Arc<Mutex<usize>>) -> bool {
        // check COUNT for completed tasks
        *counter.lock().unwrap() != find
    }
}
